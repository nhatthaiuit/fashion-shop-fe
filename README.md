...
A high-performance Single Page Application (SPA) built with React and Vite for the Fashion Shop platform, featuring a comprehensive Admin Dashboard.
...
- **Order Filtering & Search**: Advanced table filtering allowing staff to filter orders by exact status (Pending, Shipped, etc.) and perform fuzzy searches on customer names.
- **Data Export Capability**: Built-in CSV export functionality for both Products and Orders, generating structured reports directly from the dashboard.
- **Responsive E-Commerce UI**: Seamless customer experience for browsing products, viewing details, and checkout flow.
